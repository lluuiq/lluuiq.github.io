...
1. 克隆项目到该文件夹
2. 进入克隆下来的文件夹
3. 拷贝`.git`文件夹到上一级目录（即博客的根目录）
4. 返回到根目录
5. 删除`【yourusername】.github.io`文件夹
6. 返回到root文件夹
7. 重启宝塔面板
...
参考文档：[Easy Hexo](https://easyhexo.com/4-High-order-hexo-gamer/4-1-remote-editing/#三、在本地安装-hexo-admin)
...
然后在根目录创建一个文件夹存放脚本（直接放根目录也行，但后面记得更改配置的路径），名称随意，我用admin作为文件夹名，deploy作为脚本名。然后进入创建的文件夹，新建并编辑一个脚本文件
...
脚本配置好后，开启开机自动启动`hexo s`命令来启动后台。
...
然后输入命令，cd后面的路径要换成自己的博客根目录路径，例如我的为`cd /www/blog`
...
![image-20200223061352773](C:%5CUsers%5C84452%5CAppData%5CRoaming%5CTypora%5Ctypora-user-images%5Cimage-20200223061352773.png)
...
接着到宝塔面板的安全里放行4000端口，说明随意填。

![image-20200223060825403](C:%5CUsers%5C84452%5CAppData%5CRoaming%5CTypora%5Ctypora-user-images%5Cimage-20200223060825403.png)

宝塔、服务器开启4000端口，服务器的来源要是0.0.0.0/0
...
http://118.25.125.244:4000/admin

# 后记

1. 为什么要保存博客源代码？

一是备份，免得在修改主题时出错造成无法挽救的情况。二是方便迁移，以后无论是重装系统、换电脑还是换服务器都可以把设置好的博客部署出来。

2. 为什么要保存到服务器？

为了使用服务器开放hexo-admin的后台管理，做到远程、随时随地的使用后台管理。

3. 为什么要用钩子使服务器上的源代码与github同步？

后台管理进行deploy会根据服务器上的主题样式进行渲染页面，若不同步的话会用服务器上的样式，本地修改并保存的主题样式会无效。